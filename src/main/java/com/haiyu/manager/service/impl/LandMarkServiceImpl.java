...
import com.haiyu.manager.dao.dic.LandMarkMapper;
...
public class LandMarkServiceImpl implements LandMarkService {
    ...
    @Override
    public PageDataResult getLandMarkList(Integer pageNum, Integer pageSize) {
        PageDataResult pageDataResult = new PageDataResult();
        PageHelper.startPage(pageNum, pageSize);
        Example example = new Example(LandMarkDO.class);
        example.createCriteria().andEqualTo("logicDelete", 1);
        List<LandMarkDO> list = landMarkMapper.selectByExample(example);
        if(list.size() != 0){
            PageInfo<LandMarkDO> pageInfo = new PageInfo<>(list);
    ...
    @Override
    public Map<String, Object> updateLandMark(LandMarkDO landMarkDO) {
        Map<String, Object> data = new HashMap<>();
        landMarkDO.setUpdateTime(DateUtils.getCurrentDate());
        int result = landMarkMapper.updateByPrimaryKey(landMarkDO);
        try {
            if (result ==0) {
                data.put("code",0);
                data.put("msg", "更新失败!");
    ...
    public Map<String, Object> del(Integer id) {
        Map<String, Object> data = new HashMap<>();
        LandMarkDO landMarkDO = landMarkMapper.selectByPrimaryKey(id);
        landMarkDO.setLogicDelete(0);
        try {
            int result = landMarkMapper.updateByPrimaryKey(landMarkDO);
            if (result == 0) {
                data.put("code",0);
